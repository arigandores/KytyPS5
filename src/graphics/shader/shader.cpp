...
bool ShaderIsMapped(uint64_t addr) {
	...
	std::scoped_lock lock(g_shader_map_mutex);
	return g_shader_map->find(addr) != g_shader_map->end();
}
...
static bool ShaderGetStaticVertexInputInfo(uint64_t shader_addr, const HW::UserSgprInfo& user_sgpr,
	...
	// Reset() keeps the untouched array entries, but it does not list the stage members that
	// upstream cleared with `info = {}`; a reused info would otherwise inherit the mesh or
	// tessellation stage of the previous draw.
	info.logical_stage = ShaderType::Vertex;
	info.tess          = {};
...
ShaderParams PrepareProgram(const HW::VertexShaderInfo& regs, const HW::Context& context,
	...
	// Reset() only clears the members it lists, so the stage state that upstream cleared by
	// assigning {} here has to be reset explicitly.
	info.tess                = {};
...
PrepareTessellationPrograms(const HW::VertexShaderInfo& regs, const HW::Context& context,
	...
	// Session 59, B5: user_data is ShaderUserSgprs (inline array), not std::vector - the
	// upstream `insert(begin(), 8u, 0u)` has no equivalent member here; PrependZeros does the
	// same thing on our container (same call as the NGG path above).
	params[1].user_data.PrependZeros(8u);
