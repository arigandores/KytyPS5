...
Собрано в сессии 51 из чтения кода и счётчиков сессий 48–51, дополнено измерениями сессии 52.
...
| `RegionManager::Epoch()` | `regionManager.h` (сессия 52) | эпоха записи одного региона 4 МиБ |
...
материализации, ради которого она задумывалась, оказалось бесполезным (см. врезку выше).
