...
void Translator::S_WQM(const Decoder::Instruction& inst, bool wide) {
	...
	if (!wide) {
		...
		// Preserve the other EXEC/VCC half and invalidate overlapping SGPR-pair provenance.
	...
	if ((wide || program.wave_size == 32u) && program.stage == ShaderType::Pixel &&
	    inst.dst.kind == Decoder::OperandKind::ExecLo &&
		...
		// S_WQM of a pixel wave's initial EXEC (the non-helper lanes) is true for every existing
		// invocation: Vulkan creates helper invocations only inside quads that contain a covered
		// pixel, so the whole-quad expansion of "not helper" never leaves a lane out. Folding it
		// lets every top-level "exec ? new : old" VGPR write select disappear. Besides being dead
		// weight, those selects miscompiled on NVIDIA when the "old" value was a live coordinate
		// of an earlier image sample and the new value a later sample's result (ASTRO BOT video
		// PS 0xe197e307a5f2cb39 read its luma y coordinate from the register the U sample was
		// still writing).
		...
		ir.SetExecHi(program.wave_size == 64u ? IR::U32(ir.CompositeExtract(all, 1))
		                                      : IR::U32(IR::Value(0u)));
	}
}
