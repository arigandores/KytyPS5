...
**Сессия 64 — что изменилось в карте:**

- **Читающая часть разрешения привязок стоит ≈6–7 % потока, а не 20 %** (гейт `shadowinline`:
  пустыня +6,1…+7,0 % CPU на draw, 342 мкс на кадр; Sky Garden +1,7…+5,9 %, счётчиком 2,2–2,4 мс
  из 31; поровну образы/буферы). Профильные
  статьи `PrepareBindings`/`RebindImages`/`ObtainBuffer` на две трети — мутации: касания LRU,
  `ConfigureImageSource`, копии в stream-кольцо, `SynchronizeBuffer`, загрузки.
- **Чужой поток, читающий те же записи кэшей, стоит потоку GuestGpu 4–8 %** (ручка
  `shadowresolve`: пустыня K=1 +4,3 %, K=4 +7,9 %; Sky Garden K=1 +6,3 %, K=2 +3,4 %, K=4
  +4…+6,5 %), независимо от того, образы это или буферы (`shadowmask`); одна постановка задания
  в сцене — 470–500 мкс (1,5 % потока); `spin_gpu_us` 16–18 мкс на пустыне, 170–190 в сцене
  (только с пробами образов) — в основном не лок, а передача и разделяемые линии
  (`Image::binding`, тики, слоты). Те же чтения на воркере в 1,8–2,6× дороже, чем на месте.
  **Трек C (M2) закрыт потолком.**
- **Разрешимость на воркере (пустыня / сцена):** образы 96 / 94 % по записанному view
  (`texfast`); буферы — memo запроса 23 / 24 % + эпоха загрузки 9 / 5 %, копия в кольцо 19 %,
  `SynchronizeBuffer` 47 / 50 % при ~7 / ~9 буферных привязках на draw.
- **Пробуждение воркеров:** notify на каждый push = 420 мкс на кадр (7 % потока); будить пачками.
- `GameController::Disconnect` неизвестного id — лог вместо EXIT.

**Сессия 63 — что изменилось в карте:**

- **Публикация головы кольца записи стоила межчиплетного владения линией, а не xchg.** С потоком
  записи на CCD GuestGpu (гейт `recpin`, по умолчанию 1, вслед за `dapin`) `EndRecord` 4,2 → 1,1 %
  потока (xchg 4,0 → 0,9 %), ожидание дренажей в `Handle()` 150 → 83 мкс на кадр, `rec_ccd_x`
  32/41 → 0; Sky Garden −2,3…−3,6 % CPU на draw, пустыня −6,6 %. Редкая публикация (`recpubn=16`)
  снимает `EndRecord` до 0,4 %, но дренажи растут до 600 мкс (`Drain` 1,5 %) — поверх `recpin`
  разница ниже порога.
- **Прямые записи draw-потока по времени** (`FrameTrace-direct:` с микросекундами, теперь и в
  lite): с `recpubn=16 recpin` 287 на кадр и 382 мкс — закрытия прохода прямых draw 86, тайлер 72,
  `Fill` 44, прямые draw 37, клиры 34, sanitizer 28, `CopyFrom` 20, GDS-барьер 16. Первая прямая
  запись после серии записей кольца платит за весь хвост. Конвертируемые сайты ≈214 мкс (0,7 %) —
  общий механизм закрыт потолком; каталог сайтов `docs/local-session-63-direct-sites.md`.
- **Харнесс:** имена, отсутствующие в gate-файле, сохраняют текущее состояние (умолчание проверяет
  только первая фаза); семплер на пустыне нужен явный `KYTY_SAMPLE_GPU=1`.

**Сессия 62 — что изменилось в карте:**

- **Прямые записи draw-потока названы по сайтам** (`FrameTrace-direct:`, ключ — адрес возврата
  `CommandBuffer::Handle()`, `C:/kyty/s62/direct_sites.py`): в Sky Garden 800 на кадр —
  `Image::Transit` 342, прямое закрытие прохода из него и из загрузок 130, `RecordBufferCopies`
  63, `Buffer::CopyFrom` 44, `CommitBindings` 23, прямые draw 20, `Fill`, тайлер, клиры,
  sanitizer. Гейты `recimg`/`recup` переводят первые три в записи кольца (`RecordOp::ImageBarriers`,
  `RecordOp::BufferUpload`): 800 → 270, но CPU на draw ≈ −1 %, с `recpubn=16` −0,4…−2,6 % — порог
  не держат, остаются 0. С `KYTY_GPU_TIME=1` пакетов нет вовсе (3 463 прямых записи на кадр).
- **Копии гостевых диапазонов в stream-кольцо — 15–18 МБ и 10–13 тыс. на кадр**, почти всё —
  путь `ObtainBuffer` для CPU-грязных чтений ≤ 16 КБ (`ob_stream`), const-bank копии — 7,5 тыс. /
  720 КБ. 84 % копий байт-в-байт равны предыдущей копии того же диапазона, и у всех них та же
  `RangeWriteEpoch` (`cb_diff_ep` = 0 на 2,2 млн), но эпоха для грязной страницы — не свидетель
  (следующая запись гостя fault'а не делает), а проверка содержимого стоит 4,2 мс на кадр — те же
  загрузки строк, что и memcpy. Memo сэкономило бы только WC-запись в кольцо. Закрыто.
- **Кольцо stream-буфера читать нельзя:** оно в device-local host-visible памяти, чтение слота —
  PCIe на каждую строку (диагностика, читавшая тень из кольца, стоила 54 мс на кадр).
- **Вход в Sky Garden 2 из 2 без зависания**, первый — после свежей сборки с чекпоинтами.

**Сессия 61 — что изменилось в карте:**

- **Профиль потока GuestGpu после `progmemo`** (Sky Garden, `C:/kyty/s61/prof_sky61a.txt`):
  `PrepareBindings` 7,1 % (≈14 % с детьми — потолок M2), **`EndRecord` 4,9 %, из них 4,6 % — одна
  инструкция `xchg` публикации `m_head` кольца записи** (поток записи спинится на этой линии, в
  80 % проверок с другого CCD), `VirtualProtect` 4,7 %, `VerifyWitness` 4,5 %, `TryReadBacking`
  3,8 % (memcpy констант в stream-ring, `cb_copy` 7 500 на кадр / 730 КБ), `CommitBindings` 3,6 %,
  `AheadTake` 3,1 %, `GetGraphicsPrograms` 3,1 %, спин-лок трекера 2,8 % (`xchg` захвата),
  `QueueAheadSource` 1,9 % (1,2 % — промах кэша на слоте таблицы M1, 17 МБ). M1 на критическом
  потоке ≈12 %: `da_take_us` 3,5 + `da_queue_us` 0,8 + `da_walk_us` 1,8 = 6,1 мс.
- **Проверка свидетеля M1 стоит ≈2,3 мс на кадр, и её не снять префетчем.** Без проверки
  (`dawitness=0`, небезопасно) −6,9 / −3,4 % CPU на draw. Сравнение по указателям воркера с
  префетчем (`dawitptr`) экономит только ≈0,4 мс (`da_take_us` −10 %): остаток — сами чтения
  гостевых слов (≈75 тыс. прогонов, 420 тыс. слов на кадр), промахи кэша в момент проверки.
  Свидетель по эпохам регионов невозможен: у 44 % попаданий эпоха региона уже сдвинулась при
  `da_stale` = 0 (константы в регионе переписываются каждый кадр, но не те слова, что читал SRT).
- **Редкая публикация головы кольца записи не окупается** (`recpubn`): 780 прямых записей на кадр
  (`rec_direct`: загрузки буферов, барьеры, transit'ы вне пакета) дренируют поток записи перед
  каждой, и цена `xchg` переезжает в ожидание (`rec_direct_busy` 31 → 210). Чтобы взять 4,6 %
  `EndRecord`, сначала нужно убрать прямые записи из draw-потока.
- **Загрузки буферов пакетом — потолок ≈1 %:** из 65–67 загрузок на кадр 39–40 идут подряд без
  draw, но разрывов прохода по ним всего 16–19 (`FrameTrace-rp: end_buf_upload`), перезапусков
  14–17.
- **Первый вход в Sky Garden после новой сборки зависает на первом тяжёлом кадре уровня** (2 из
  4 заходов сессии 61, `nvlddmkm 153`, `acopy` в порядке, отправки возвращаются, GPU не завершает
  тик); повторный вход с тем же бинарём проходит (3 из 3). То же правило — сессия 54; подпись кэша
  трансляции при этом не менялась, механизм не назван.

**Сессия 60 — что изменилось в карте:**

- **Поиск программ и пайплайна на неизменных регистрах — ≈9 % потока, 75 % draw'ов его не
  нуждаются.** Гейт `progmemo` (по умолчанию 1): свидетель — все входы `PrepareProgram` байтами
  (регистры стадии, `HW::ShaderRegisters`, стадии шейдеров, clip control/viewport 0, provoking
  vertex, prim type, `GeControl`, export mapping, эпоха регистрации шейдеров) и слова таблиц
  атрибутов/V#, прочитанные `ShaderApplyAttribSemantics`; значения user SGPR в свидетель не входят
  (копируются на попадании). Попадание пропускает `PrepareProgram`, `BuildStageStaticKey`,
  `programs.find` и хэш+поиск `GraphicsPipelineKey`; проверка ≈60 нс на draw. Sky Garden
  −1,3…−2,9 % CPU на draw. Важно: `ProgramCache::lookup_key` теперь три ключа по слотам (VS, PS,
  прочее), карта `programs` несёт `programs_epoch`.
- **Защита страниц после загрузки — статья с потолком 1,6 мс на кадр (5 %), которую нельзя
  снять без двойной копии.** На потоке GuestGpu 195 вызовов `VirtualProtect` на кадр / 6 500
  страниц, 90 % — RO-вооружение после загрузок только для чтения (159 вызовов, 5 900 страниц);
  `ApplyGuard` там ждёт всего ≈0,1 мс. Перенос `VirtualProtect` на воркер (`armdefer`) требует
  держать страницу грязной до подтверждённой защиты и копировать её второй раз — на пустыне это
  +2,3 МБ и +22 загрузки на кадр (каждая рвёт render pass) и +4,5…+7,8 % CPU на draw. Загрузки
  буферов (72 на кадр, 23 МБ в Sky Garden) — сами по себе следующая статья: каждая
  `EndRendering` + два барьера + `copyBuffer`.
- **Обход PM4 для M1 на своём потоке не окупается и с удержанием** (`dawalklead=1/2`): регрессии
  нет, но `da_take_us` +15–25 % и `da_miss` ×1,3–2,3 съедают перенос обхода. Закрыт.
