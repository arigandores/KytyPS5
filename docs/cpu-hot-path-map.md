...
Собрано в сессии 51 из чтения кода и счётчиков сессий 48–51. Цель документа — не искать это
заново: где именно уходит CPU на кадр и какие кэши/эпохи уже есть в дереве.
...
| `GuestBackingStore::MapEpoch()` | `memoryAddressSpace.inc` (сессия 51) | постоянные трансляции страниц SRT |
| `Common::Gates` | `src/common/gates.*` (сессия 51) | A/B опций внутри одного прогона (`KYTY_GATE_FILE`) |

Чего нет: эпохи на регион трекера (нужна для memo материализации), эпохи GPU-dirty состояния
текстур, стабильных смещений stream-ring для `flattened_srt`/`shader_data`.
