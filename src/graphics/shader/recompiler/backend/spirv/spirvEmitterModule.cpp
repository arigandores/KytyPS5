...
void DefineDescriptors(EmitterState& state) {
	...
	for (const auto& binding: state.program.bindings.descriptors) {
		...
		switch (binding.kind) {
			case IR::DescriptorBindingKind::Buffers: {
				...
				if (state.requirements.buffer_int64_atomics) {
					state.storage_buffer_u64_variable =
					    Alias(StorageBufferU64Type(state), "buffers_u64");
				}
				// uvec4/uvec2 views of the same descriptors: one load per scalar group.
				if (state.requirements.scalar_vector_loads) {
					state.storage_buffer_u32x4_variable =
					    Alias(StorageBufferU32VectorType(state, 4u), "buffers_u32x4");
			...
			case IR::DescriptorBindingKind::ConstBuffers: {
				// Uniform-buffer views of the const-bank V#s (constant bank on NVIDIA).
				const auto count = static_cast<uint32_t>(binding.resources.size());
				const auto DefineBlock = [&](uint32_t components, const char* name) {
				...
				if (state.requirements.scalar_vector_loads) {
					state.const_buffer_u32x2_variable = DefineBlock(2u, "cbuffers_u32x2");
					state.const_buffer_u32x4_variable = DefineBlock(4u, "cbuffers_u32x4");
...
void DefineInputs(EmitterState& state) {
	...
	// Pixel inputs that address the same vertex parameter share one variable: the vertex
	// shader exports each parameter to a single location. Resolve the alias groups first so
	// that an owner's type already reflects a per-vertex alias of the same location.
	if (state.program.stage == ShaderType::Pixel) {
		for (size_t index = 0; index < state.inputs.size(); index++) {
			...
			const auto location = PixelParameterMappedLocation(state, input.location);
			for (size_t prior = 0; prior < index; prior++) {
				...
				if (candidate.kind == IR::StageInputKind::Parameter && !candidate.alias &&
					...
					candidate.per_vertex |= input.per_vertex;
					input.alias = true;
					break;
				}
			}
	...
	for (auto& input: state.inputs) {
		if (input.alias) {
			continue; // shares the owner's variable, annotations and interface entry
		}
	...
	for (auto& input: state.inputs) {
		...
		for (const auto& owner: state.inputs) {
			if (!owner.alias && owner.kind == IR::StageInputKind::Parameter &&
			    PixelParameterMappedLocation(state, owner.location) == location) {
				input.variable_id = owner.variable_id;
				input.per_vertex  = owner.per_vertex;
				break;
			}
		}
	}
...
void DefineModule(EmitterState& state) {
	...
	if (DenormFlushToZeroDeclared()) {
		state.builder.RequireCapability(spv::CapabilityDenormFlushToZero);
	}
