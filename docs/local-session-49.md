# Сессия 49 — Sky Garden: исправлены цвет и исчезновение листвы

2026-09-12, база `6f67816`, ветка `merge-upstream`. Сессия завершена по указанию
пользователя; **FPS продолжаем в следующей сессии, 60 FPS пока не достигнуты**.
Код CMASK — `1076032`; все изменения сохранены локально, без push.

## Итог и проверка

- Избыточная яркость была связана с потерей актуального GPU-образа при GC на фоне
  старого грязного buffer-alias; сохранение образа исправило цвет, пользователь подтвердил.
- Исчезновение кроны прослежено через RenderDoc: 4872/4872 вершины становились NaN
  после чтения поля деформации. В поле оставались 36864 пикселя из старой HTile-заливки
  `0xFFFFFFF0` (frame4, адрес `0x516870000`, размер `0x48000`).
- Причина пропущенной очистки — CMASK, отсутствовавшая в описании цветовой поверхности.
  Поле `0x516830000` использует CMASK `0x56c05e000` и CLEAR_WORD0/1
  `38003800/00005650`, то есть RGBA16F `(0.5, 0.5, 101, 0)`.
  Добавлена штатная очистка перед использованием, включая eliminate до первого draw,
  проверку полного заполнения метаданных и однократное применение к слою.
  Поддержаны single-sample/single-mip 2D поверхности целыми блоками 1024x1024;
  маленькие, дополненные до блока и MSAA-размещения не заявлены поддержанными.
  Контроль — `KYTY_CMASK_CLEAR=0`. Принудительного NaN-clamp в шейдерах нет.
- Пользователь: «да, деревья исправлены вроде». Проверка видео того же дерева справа
  под платформой: кадры **15648–15708**, исчезновения нет; обзор **15614–15774**
  также сохраняет крону. Артефакты `cmask_crown_frames.jpg`, `cmask_flight_alignment.jpg`
  извлечены из видео, это не проверка отдельными live-скриншотами.
- Финальное видео `C:/kyty/s49/rec_cmask_fix.mp4`: **16657 кадров**, decode exit0,
  глобальный A-B-A сканер — 0 кандидатов. Это не доказательство отсутствия всех локальных
  дефектов; отмеченный участок проверен отдельно.
- Финальный прогон `cmask_fix`: старт23:19:11, WM_CLOSE23:25:03, exit0,
  первый Mesh15534, последний FrameTrace16645; зависания нет. Игра закрыта.
  Окна +120/+420/+720 по300present: **13.31/13.69/13.79fps**,
  CPU медиана **77.9/67.4/67.2мс**, GPU около **22.1мс**, 5551–5870draw.
  Это диагностический запуск; рост относительно других запусков не приписывается CMASK.
- GPU-тест CMASK прошёл обычную Vulkan и synchronization validation: старые NaN,
  неполная заливка, оба слова цвета, дальний угол, повторная привязка, expanded-код,
  новая очистка и eliminate до создания образа. Контроль0 воспроизводит ошибку.
  DCC fixed-float и sampled-clear прошли регрессию. Большая suite не объявляется чистой:
  старый storage-mip00958 и partial-page readback0/0 воспроизводятся и с контрольными путями.
  Полный игровой маршрут под Vulkan/sync validation после CMASK не запускался.

Дополнительно сохранены resident-mip source ranges, GC progress/guard/preservation,
диагностика зависаний и исправление потери других выходов у mixed metadata-clear dispatch
(`0fac81f`, тест ON проходит, контроль0 падает; в проверенном маршруте mixed-случаи не найдены).
Native SRT остаётся opt-in, default0; общий FPS-выигрыш этой ветки не доказан.
Редкий hang входа НЕ объявляется исправленным. Watchdog не обращается к драйверу:
после остановки кадров снимает CPUdump/stacks и завершает процесс; F1 пользователя исключён.

Установленный бинарь совпадает с build/install, SHA256
`7DDFE9C7886561D5BE2863B7AA56FBA46E50D0FE2E32608859DF92FA50693206`.
Seed в этой сессии не обновлялся; холодное восстановление нового полного маршрута не проверено.
Посторонние удаления PNG в подмодуле nlohmann_json не затронуты.

## Следующая сессия — FPS

1. Сохранить исправления CMASK/GC; не возвращаться к LOD/принудительной альфа-маске:
   исходный leafPS воспроизводится побайтово, AlphaToMask отключена самим гостем.
2. Снять полный CPU-профиль (`KYTY_FRAME_TRACE=1`, не lite) уже известного входного
   пролёта с птицами. Разобрать SRT/EvaluateCompiled, привязки ресурсов и стоимость
   нескольких тысяч draw. После изменения сравнить одинаковые окна от MeshRestart.
   Нули детальных таймеров в `profile_cmask_comparison.json` означают lite-режим,
   а не отсутствие затрат. GPU~22мс тоже выше бюджета60fps.
3. Не считать обычные профили идеальным A/B: диагностика, нагрев, число draw различаются.
   Ориентир текущего диагностического прогона13–14fps; стабильных60 пока нет.
4. Маршрут вводит пользователь, ввод не записывать; RenderDoc F1 только пользователь.
   Закрывать игру после нужного замера. Сохранять watcher и точный linker map для hang.

Рабочие параметры последнего прогона: ASYNC_COPY_GPU_WAIT=0 (не доказанный фикс hang),
SRT_NATIVE=0, DYNAMIC_STATE_CACHE=0, RESIDENT_MIPS=1, IMAGE_GC_PROGRESS=1,
IMAGE_GC_FRAME_GUARD=1, IMAGE_GC_PRESERVE_GPU=1, CMASK_CLEAR=1, GPU_CHECKPOINTS=nv.
Для профиля очистить опциональные CLEAR_TRACE/IMAGE_WATCH и использовать осознанный
контроль instrumentation. Все логи, видео, maps и скрипты — `C:/kyty/s49/`.

## Ход исследования

Ниже сохранены ранние checkpoints и промежуточные гипотезы; актуальный итог указан выше.
