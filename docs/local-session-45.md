...
- Обзор видео на одно-кадровые глитчи не завершён: первый прогон скрипта упал по памяти
  (запускался одновременно с игрой), повторный на момент отчёта ещё считался. Чистым видео не объявляется.
