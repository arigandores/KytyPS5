...
uint32_t ImmediateAddress(EmitterState& state, uint32_t address, int32_t immediate) {
	return immediate == 0
	           ? address
	           : Binary(state, OpIAdd, TypeScalarU64(state), address,
...
uint32_t LoadScalarBda(ValueEmitContext& ctx, const IR::Inst& inst, const IR::MemoryInfo& mem,
	...
	if (!use_cache || mem.address_is_full || handle == nullptr ||
		...
		const auto address = Binary(state, OpBitwiseAnd, TypeScalarU64(state),
	...
	if (cache.handle != handle || cache.low != low || cache.block != ctx.state.current_block) {
		...
		auto address = Binary(state, OpIAdd, TypeScalarU64(state), base,
		                      Unary(state, OpUConvert, TypeScalarU64(state), masked_low));
		address      = Binary(state, OpBitwiseAnd, TypeScalarU64(state), address,
		                      ConstantDeviceAddress(state, ~uint64_t {3}));
		cache.handle   = handle;
		cache.low      = low;
	...
	const auto page_offset =
	    Binary(state, OpBitwiseAnd, TypeScalarU64(state), cache.address,
...
uint32_t LoadFlatDwordNullPage(ValueEmitContext& ctx, uint32_t address, uint32_t active_id,
                               uint32_t bits) {
	auto&      state   = ctx.state;
	const auto aligned = Binary(state, OpBitwiseAnd, TypeScalarU64(state), address,
	                            ConstantDeviceAddress(state, ~uint64_t {3}));
		...
		const auto second = EmitValueOrZeroIfCondition(state, crosses, [&]() {
			return LoadBdaAt(
			    ctx, GetBdaPointer(ctx,
			                       Binary(state, OpIAdd, TypeScalarU64(state), aligned,
...
uint32_t EmitBvhIntersectRay(ValueEmitContext& ctx, const IR::Inst& inst) {
	...
	const auto addrt = TypeScalarU64(state);
...
void EmitReadConstBuffer(ValueEmitContext& ctx, const IR::Inst& inst) {
	...
	// Already covered by a grouped scalar load (KYTY_SCALAR_GROUP).
	if (ctx.grouped_loads.contains(&inst)) return;
	auto& state = ctx.state;
	...
	if (UsesConstBank(state, mem.resource)) {
		...
		const auto buffer_slot =
		    ResourceForDescriptor(state, IR::DescriptorBindingKind::Buffers, mem.resource);
		const auto byte = Binary(state, spv::OpIAdd, TypeU32(state), address,
