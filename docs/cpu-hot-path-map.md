...
- **Читающая часть разрешения привязок стоит ≈6 % потока, а не 20 %** (пустыня, гейт
  `shadowinline`: +6,1…+7,0 % CPU на draw, 342 мкс на кадр, поровну образы/буферы). Профильные
...
- **Чужой поток, читающий те же записи кэшей, стоит потоку GuestGpu 4–8 %** (ручка
  `shadowresolve`, K=1 +4,3 %, K=4 +7,9 %), независимо от того, образы это или буферы
  (`shadowmask`); `spin_gpu_us` при этом 16–18 мкс — не лок, а передача задания и разделяемые
  линии (`Image::binding`, тики, слоты). Те же чтения на воркере в 2,6× дороже, чем на месте.
- **Разрешимость на воркере (пустыня):** образы 96 % по записанному view (`texfast`); буферы —
  memo запроса 23 % + эпоха загрузки 9 %, копия в кольцо 19 %, `SynchronizeBuffer` 47 %.
