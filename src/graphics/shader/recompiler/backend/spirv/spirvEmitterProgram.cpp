...
bool EmitLoopGuard(ValueEmitContext& ctx, const IR::BlockInfo& info, uint32_t variable) {
	...
	state.builder.AddFunction({OpLoopMerge, ctx.Label(merge), ctx.Label(cont), spv::LoopControlMaskNone});
	...
	state.builder.AddFunction({OpSelectionMerge, after_label, spv::SelectionControlMaskNone});
...
void EmitStructuredTerminator(ValueEmitContext& ctx, const IR::Block* block,
	...
	const auto  emit_merge = [&]() {
		if (term.loop_header && !guarded) {
			...
			const auto* merge = (!guarded && term.merge_block != UINT32_MAX)
			                        ? TargetBlock(program, term.merge_block)
			                        : nullptr;
			if (merge != nullptr) {
				ctx.state.builder.AddFunction(
				...
				ctx.state.builder.AddFunction({spv::OpSelectionMerge, label, spv::SelectionControlMaskNone});
			}
		}
	};
	...
	switch (term.kind) {
		case CFG::TerminatorKind::Branch: {
			...
			if (target == nullptr ||
			    (!term.loop_header && IsEmptyTerminalBlock(program, term.true_block))) {
				EmitReturn(ctx);
				return;
			}
			emit_merge();
